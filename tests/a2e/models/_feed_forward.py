...
class TestUtility(unittest.TestCase):
    ...
    def test_create_deep_feed_forward_autoencoder_shallow(self):
        ...
        self.assertEquals(model.layers[0].input_shape[0][1], 100)
        self.assertEquals(model.layers[1].output_shape[1], 50)
        self.assertEquals(model.layers[2].output_shape[1], 100)
    ...
    def test_create_deep_feed_forward_autoencoder_deep(self):
        ...
        self.assertEquals(len(model.layers), 5)
        self.assertEquals(model.layers[0].input_shape[0][1], 120)
        self.assertEquals(model.layers[1].output_shape[1], 60)
        self.assertEquals(model.layers[2].output_shape[1], 30)
        self.assertEquals(model.layers[3].output_shape[1], 60)
        self.assertEquals(model.layers[4].output_shape[1], 120)
