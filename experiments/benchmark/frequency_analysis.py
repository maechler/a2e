...
def run_callable(run_config: dict):
    ...
    for i, (defect_type, defect_frequency_order) in enumerate(config['defect_frequency_orders'].items(), 0):
        ...
        vlines.append({'x': defect_frequency, 'color': config['colors'][i], 'label': f'{defect_type} = {"{:.2f}".format(defect_frequency)}Hz', 'linestyle': 'dashed'})

        if config['show_bandwidth']:
            vlines.append({'x': int(defect_frequency) + bandwidth, 'color': config['colors'][i], 'label': '', 'linestyle': 'dashed'})
            vlines.append({'x': max(int(defect_frequency) - bandwidth, 0), 'color': config['colors'][i], 'label': '', 'linestyle': 'dashed'})
    ...
    experiment.plot(y=train_median.values[:250], vlines=vlines, ylim=ylim, xlabel='frequency [Hz]',  ylabel='amplitude', title=f'{run_config["data_set"]} (train)', key='train_' + data_set_key)
    experiment.plot(y=test_healthy_median.values[:250], vlines=vlines, ylim=ylim, xlabel='frequency [Hz]',  ylabel='amplitude', title=f'{run_config["data_set"]} (healthy)', key='healthy_' + data_set_key)
    experiment.plot(y=test_anomalous_median.values[:250], vlines=vlines, ylim=ylim, xlabel='frequency [Hz]',  ylabel='amplitude', title=f'{run_config["data_set"]} (anomalous)', key='anomalous_' + data_set_key)
